...
''' Activation Functions
    Input: batchX, maybe is a element, a array, or a matrix
'''

class Sigmoid:

	def primitive(self, batchX):
		return  return 1.0 / (1.0 + np.exp(-batchX))

	def derivative(self, batchX):
		return (1.0 - x) * x
...
class Softmax:
	...
	def derivative(self, xlists, j, i=j):
		elist = np.exp(xlists)
		s = np.sum(elist)
		aj = 1.0 * elist[j] / s 
		if j == i:
